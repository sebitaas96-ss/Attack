...
import adminMenuItems, { IAdminMenuItem} from 'src/app/constants/adminMenu';
...
export class AdminSidebarComponent implements OnInit, OnDestroy {
  adminMenuItems: IAdminMenuItem[] = adminMenuItems;
  selectedParentMenu = '';
  viewingParentMenu = '';
  currentUrl: string;
  ...
  findParentInPath(path): any {
    const foundedMenuItem = this.adminMenuItems.find((x) => x.to === path);
    if (!foundedMenuItem) {
  ...
  isCurrentMenuHasSubItem(): boolean {
    ...
    const menuItem = this.adminMenuItems.find(
  ...
  openSubMenu(event: { stopPropagation: () => void }, menuItem: IAdminMenuItem): void {
    if (event) {
  ...
  filteredMenuItems(menuItems: IAdminMenuItem[]): IAdminMenuItem[] {
    return menuItems
      ? menuItems.filter(
          (x) =>
